...
from sglang.srt.managers.scheduler import run_scheduler_process, run_scheduler_process_sim
from sglang.srt.managers.template_manager import TemplateManager
from sglang.srt.managers.tokenizer_manager import TokenizerManager
from sglang.srt.server_args import PortArgs, ServerArgs, SimArgs, SimBinds
...
class EngineSim(Engine):
    def __init__(self, **kwargs):
        sim_args = kwargs.pop("sim_args", None)
        logger.info(f"{sim_args=}")
        ...
        # Launch subprocesses
        tokenizer_manager, template_manager, scheduler_info = _launch_subprocesses_sim(
            server_args=server_args,
            port_args=self.port_args,
            sim_args=sim_args,
        )
...
def _launch_subprocesses_sim(
    server_args: ServerArgs, sim_args: SimArgs, port_args: Optional[PortArgs] = None
) -> Tuple[TokenizerManager, TemplateManager, Dict]:
    """
    Launch the TokenizerManager in the main process, the Scheduler in a subprocess, and the DetokenizerManager in another subprocess.
    """
    ...
    if server_args.dp_size == 1:
        ...
        for pp_rank in pp_rank_range:
            for tp_rank in tp_rank_range:
                ...
                moe_ep_rank = tp_rank // (server_args.tp_size // server_args.ep_size)
                proc = mp.Process(
                    target=run_scheduler_process_sim,
                    args=(
                        server_args,
                        port_args,
                        sim_binds,
