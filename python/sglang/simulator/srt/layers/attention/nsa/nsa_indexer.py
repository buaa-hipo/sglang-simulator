...
from sglang.simulator.srt.layers.layernorm import LayerNorm
from sglang.simulator.srt.utils import add_prefix, ceil_align, is_cuda, is_hip, is_npu
...
if TYPE_CHECKING:
    from sglang.simulator.srt.mem_cache.memory_pool import NSATokenToKVPool
...
class Indexer(CustomOp):
    ...
    def forward_indexer(
        ...
        if not is_npu():
            from sglang.simulator.srt.layers.attention.nsa.tilelang_kernel import fp8_index
    ...
    def forward_cuda(
        ...
        if is_hip():
            from sglang.simulator.srt.layers.attention.nsa.tilelang_kernel import act_quant
        elif not is_npu():
            from sglang.simulator.srt.layers.attention.nsa.triton_kernel import act_quant
