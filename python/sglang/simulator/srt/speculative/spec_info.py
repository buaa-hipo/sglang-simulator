...
def register_speculative_algorithm(
    ...
    *,
    ...
    Example:
        >>> from sglang.simulator.srt.speculative.spec_info import register_speculative_algorithm
...
def _create_eagle_worker(**kwargs: Any) -> Any:
    ...
    if enable_overlap:
        from sglang.simulator.srt.speculative.eagle_worker_v2 import EAGLEWorkerV2

        return EAGLEWorkerV2(**kwargs)

    from sglang.simulator.srt.speculative.eagle_worker import EAGLEWorker

    return EAGLEWorker(**kwargs)


def _create_standalone_worker(**kwargs: Any) -> Any:
    from sglang.simulator.srt.speculative.standalone_worker import StandaloneWorker

    return StandaloneWorker(**kwargs)


def _create_ngram_worker(**kwargs: Any) -> Any:
    from sglang.simulator.srt.speculative.ngram_worker import NGRAMWorker
