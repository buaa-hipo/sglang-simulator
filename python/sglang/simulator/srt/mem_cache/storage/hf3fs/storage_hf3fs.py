...
from sglang.simulator.srt.mem_cache.storage.hf3fs.hf3fs_client import Hf3fsClient
from sglang.simulator.srt.metrics.collector import StorageMetrics
...
def create_hf3fs_client(
    ...
    """
    if use_mock:
        from sglang.simulator.srt.mem_cache.storage.hf3fs.hf3fs_client import Hf3fsMockClient

        logger.info(f"[Rank Using Hf3fsMockClient for testing")
        return Hf3fsMockClient(path, size, bytes_per_page, entries)
    else:
        from sglang.simulator.srt.mem_cache.storage.hf3fs.hf3fs_usrbio_client import (
            Hf3fsUsrBioClient,
        )
...
class HiCacheHF3FS(HiCacheStorage):
    ...
    def from_env_config(
        ...
        """
        from sglang.simulator.srt.mem_cache.storage.hf3fs.mini_3fs_metadata_server import (
